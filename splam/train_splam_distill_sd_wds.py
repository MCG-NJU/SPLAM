...
# Fomulation for interpolated results, see supplementary materials
def predicted_origin_splam(model_output, start_timesteps, timesteps, sample, prediction_type, alphas, sigmas, gammas):
    if prediction_type == "epsilon":
        start_sigmas = extract_into_tensor(sigmas, start_timesteps, sample.shape)
        start_alphas = extract_into_tensor(alphas, start_timesteps, sample.shape)
        end_sigmas = extract_into_tensor(sigmas, timesteps, sample.shape)
        end_alphas = extract_into_tensor(alphas, timesteps, sample.shape)
        # inter_sigmas = (gammas * start_sigmas) + ((1 - gammas) * end_sigmas)
        inter_sigmas = (end_sigmas * start_alphas / end_alphas) ** 2 + (gammas ** 2) * (1 - (start_alphas / end_alphas) ** 2)
        inter_sigmas = inter_sigmas.sqrt()
        pred_x_0 = (sample -  inter_sigmas * model_output) / start_alphas
    else:
        raise ValueError(f"Prediction type {prediction_type} currently not supported.")
...
def parse_args():
    ...
    # ----Sub-path Linear Approx Distillation (SPLAD) Specific Arguments----
    parser.add_argument(
        "--w_scale",
        type=float,
        default=8.0,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                w = args.w_scale * torch.ones((bsz,))
                w = w.reshape(bsz, 1, 1, 1)
                # Move to U-Net device and dtype
                w = w.to(device=latents.device, dtype=latents.dtype)

                # 2.4.7. Sample linear interpolation on subpath
                gamma = torch.rand((bsz,))
                # rescale gamma for embedding, to be compatible with LCM interface
                gamma_index = 10 - gamma * 10
                gamma_embeds = guidance_scale_embedding(gamma_index, embedding_dim=args.unet_time_cond_proj_dim)
                gamma_embeds = gamma_embeds.to(device=latents.device, dtype=latents.dtype)
                gamma = gamma.to(device=latents.device, dtype=latents.dtype)
                gamma = gamma.reshape(bsz, 1, 1, 1)

                # constant embedding for target_unet (gamma=1)
                constant_embeds = guidance_scale_embedding(torch.zeros((bsz,)), embedding_dim=args.unet_time_cond_proj_dim)
                constant_embeds = constant_embeds.to(device=latents.device, dtype=latents.dtype)

                # 20.4.8. Prepare prompt embeds and unet_added_conditions
                prompt_embeds = encoded_text.pop("prompt_embeds")

                # 20.4.9. Use the ODE solver to predict the kth step in the augmented PF-ODE trajectory after
                ...
                # 20.4.10. Get target LCM prediction on x_prev, w, c, t_n
                with torch.no_grad():
                    with torch.autocast("cuda", dtype=weight_dtype):
                        target_noise_pred = target_unet(
                            x_prev.float(),
                            timesteps,
                            timestep_cond=constant_embeds,
                ...
                # perform interpolation between z_{t_{n+k}}(noisy_model_input) and z_{t}(x_prev), with drift effect
                start_alphas = extract_into_tensor(alpha_schedule, start_timesteps, latents.shape)
                end_alphas = extract_into_tensor(alpha_schedule, timesteps, latents.shape)
                inter_input = gamma * noisy_model_input + (1 - gamma) * x_prev * start_alphas / end_alphas

                # 20.4.11. Get online LCM prediction on z_{t_{n + k}, gamma}, w, c, t_{n + k}, gamma
                noise_pred = unet(
                    inter_input,
                    start_timesteps,
                    timestep_cond=gamma_embeds,
                    encoder_hidden_states=prompt_embeds.float(),
                ).sample

                pred_x_0 = predicted_origin_splam(
                    noise_pred,
                    start_timesteps,
                    timesteps,
                    inter_input,
                ...
                model_pred = c_skip_start * inter_input + c_out_start * pred_x_0
